...
class Element:
    ...
    def click_with_offset(self, *, xoffset: int, yoffset: int, timeout=None):
        builder = ActionChains(self.page.browser).move_to_element_with_offset(
            to_element=self.find(timeout=timeout), xoffset=xoffset, yoffset=yoffset
        ).click()
        builder.perform()
...
class BasePage:
    ...
    first_google_suggestion = Element('.pac-item', Element.By.CSS_SELECTOR, wait=8)
    first_scoperty_suggestion = Element('.mat-option', Element.By.CSS_SELECTOR, wait=8)
    ...
    def visit(self, **kwargs):
        if kwargs:
            ...
            query_params_string = ''
        self.browser.get(FRONTEND_BASE_URL + self.URL + query_params_string)
